...
const ImagesliderVarient = ({ 
  ...
  // Update images when selected color changes or imageList prop changes
  useEffect(() => {
    if (imageList && imageList.length > 0) {
      setCurrentImages(imageList);
      setImageSelected(imageList[0]?.path || imageList[0]?.url || "");
      setActiveIndex(0);
    }
  ...
  const imageOnClickHandler = (id) => {
    if (currentImages[id]) {
      setImageSelected(currentImages[id].path || currentImages[id].url || "");
      setActiveIndex(id);
      scrollToTop();
    }
  };
