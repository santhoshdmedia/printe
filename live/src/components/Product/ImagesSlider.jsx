...
const ImagesSlider = ({ imageList = [], data = {} }) => {
    ...
    hasMultipleImages && (
      <Tooltip title={`${isAutoPlaying ? 'Pause' : 'Play'} slideshow`}>
        <button
          ...
          className={`absolute bottom-4 right-4 p-2 rounded-full z-30 shadow-md hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white ${
            isAutoPlaying 
  ...
  const renderWishlistButton = () => (
    ...
    >
      <button
        className={`absolute top-4 right-4 p-2 w-10 h-10 !border-none bg-transparent rounded-full shadow-md hover:shadow-lg transition-all duration-200 z-40 focus:outline-none ${
  ...
  return (
    <div className="!sticky !top-24 w-full h-full" role="region" aria-label="Product images">
