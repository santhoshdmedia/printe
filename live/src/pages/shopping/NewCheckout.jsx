...
// import { Button, Checkbox, Collapse, Divider, Drawer, Form, Input, Modal, Radio, Select, Spin, Table, Tag, Tooltip, Switch } from "antd";
// import { emailValidation, formValidation } from "../../helper/form_validation";
// import { IconHelper } from "../../helper/IconHelper";
// import { CUSTOM_ERROR_NOTIFICATION, CUSTOM_SUCCESS_SWALFIRE_NOTIFICATION, ERROR_NOTIFICATION, SUCCESS_NOTIFICATION } from "../../helper/notification_helper";
// import { craeteOrderId, createOrder, getMyShoppingCart, removeMyShoppingCart } from "../../helper/api_helper";
...
//         const filteredItems = allCartItems.filter(item => selectedIds.includes(item._id));
...
//               <img src={image} alt={record.product_name} className="!h-[80px] !w-[80px] object-contain" />
//             </Link>
//           </Tooltip>
//           <div className="ml-4">
...
//       key: "product_price",
//       render: (price) => <span className="font-medium">₹{Number(price).toFixed(2)}</span>,
//       align: 'right',
//       width: '15%'
//     },
//     {
...
//       key: "final_total",
//       render: (final_total) => <span className="font-bold">₹{Number(final_total).toFixed(2)}</span>,
//       align: 'right',
//       width: '15%'
//     },
//   ];
...
//     const orderPromises = cardData.map(async (product) => {
//       const orderData = {
//         delivery_address: {
...
//         cart_items: [product],
//         total_price: Number(product.final_total),
//         gst_no: gstNo,
...
//     try {
//       if (!acceptTerms) {
//         return CUSTOM_ERROR_NOTIFICATION("Please confirm you have accept our terms and condition");
//       }
...
//       const totalAmount = Math.round(GET_TOTAL_AMOUNT() * 100);
...
//             }
//           } catch (err) {
//             ERROR_NOTIFICATION("Payment verification failed. Please contact support if amount was deducted.");
//             console.error(err);
//           }
//         },
//         name: "PRINTE",
...
//         modal: {
//           ondismiss: function() {
//             CUSTOM_ERROR_NOTIFICATION("Payment window was closed. Please try again if you want to proceed with online payment.");
//           }
//         }
//       };
...
//         Alternate_mobile_number: addresses[selectedAddress].alternateMobileNumber,
...
//           {/* Header */}
//           <div className="mb-4">
//             <Breadcrumbs title={"Shopping cart"} title2={"Checkout"} titleto={"/shopping-cart"} className="mt-2" />
//           </div>
...
//                     <Form.Item label="Name" name="name" rules={formValidation("Please Enter Name")}>
//                       <Input placeholder="Full name" size="large" />
//                     </Form.Item>
//                     <Form.Item label="Mobile Number" name="mobile_number" rules={formValidation("Please Enter Mobile Number")}>
//                       <Input type="number" placeholder="Mobile number" size="large" />
//                     </Form.Item>
//                     <Form.Item label="Email" name="email" rules={emailValidation()}>
//                       <Input placeholder="Email address" size="large" />
//                     </Form.Item>
//                     <Form.Item label="Alternate Mobile Number" name="Alternate_mobile_number">
//                       <Input type="number" placeholder="Alternate number" size="large" />
//                     </Form.Item>
//                     <Form.Item label="Street Address" name="street_address" rules={formValidation("Please Enter Street Address")}>
//                       <Input.TextArea placeholder="Full address" rows={2} />
//                     </Form.Item>
//                     <Form.Item label="Pincode" name="pincode" rules={formValidation("Please Enter Pincode")}>
//                       <Input type="number" placeholder="Pincode" size="large" />
//                     </Form.Item>
//                   </div>
//                 </Form>
//               </div>
...
//                         <div className="text-sm text-gray-600">Pay using Razorpay secure payment gateway</div>
//                       </div>
//                     </div>
//                   ))}
//                 </div>
...
//                 <h2 className="text-lg font-bold mb-4">3. GST Information (Optional)</h2>
...
//                     maxLength={15} 
//                   />
//                   <Button type={isEditable ? "primary" : "default"} onClick={handleEdit} size="large">
//                     {isEditable ? "Save" : "Edit"}
//                   </Button>
//                   {isEditable && (
//                     <Button type="default" onClick={() => setIsEditable(false)} size="large">
//                       I don't have GSTIN
//                     </Button>
//                   )}
//                 </div>
//                 {gstNo && gstNo.length !== 15 && (
//                   <div className="text-red-500 text-sm mt-2">GST number must be exactly 15 characters long.</div>
//                 )}
//               </div>
//             </div>
...
//                     <span>Subtotal ({cardData.length} items):</span>
//                     <span>₹{Number(GET_SUB_TOTAL()).toFixed(2)}</span>
//                   </div>
//                   <div className="flex justify-between mb-2">
//                     <span>CGST (4%):</span>
//                     <span>₹{Number(GET_CST_TOTAL("cgst")).toFixed(2)}</span>
//                   </div>
//                   <div className="flex justify-between mb-2">
//                     <span>SGST (4%):</span>
//                     <span>₹{Number(GET_CST_TOTAL("sgst")).toFixed(2)}</span>
//                   </div>
...
//         {orderSuccess && <Confetti style={{ zIndex: 9999, height: "100%", width: "100%" }} friction={0.99} />}
//       </div>
...
//         }}
//       >
//         <Information data={termsAndConditions} subHeadingAvailable={true} breadcrumbs={true} />
//       </Drawer>
...
//     return _.sum(
//       cardData.map((res) => {
//         return parseFloat(res.final_total);
//       })
//     );
//   };
...
//     return _.sum(
//       cardData.map((res) => {
//         return value === "cgst" ? parseFloat(res.cgst) : parseFloat(res.sgst);
//       })
//     );
//   };
...
//     const cgst = GET_CST_TOTAL("cgst");
//     const sgst = GET_CST_TOTAL("sgst");
//     return subtotal + cgst + sgst;
//   };

//   const GET_PAYABLE_AMOUNT = () => {
//     const total = GET_TOTAL_AMOUNT();
//     return paymentOption === "half" ? total * 0.5 : total;
//   };
...
//         payment_type: paymentType,
//         cart_items: cardData,
//         total_price: GET_TOTAL_AMOUNT().toFixed(2),
//         gst_no: gstNo,
//         payment_id: values.payment_id,
//         transaction_id: values.transaction_id,
//         payment_status: paymentOption === "full" ? "completed" : "advance_paid",
//         payment_amount: GET_PAYABLE_AMOUNT().toFixed(2),
//         remaining_amount: paymentOption === "half" ? 
//           (GET_TOTAL_AMOUNT() - GET_PAYABLE_AMOUNT()).toFixed(2) : 
//           "0.00",
...
//       CUSTOM_SUCCESS_SWALFIRE_NOTIFICATION({
//         title: `Your order has been successfully placed with ${paymentOption === "full" ? "full" : "50%"} payment`,
//         icon: "success",
//         confirmButtonText: "My orders",
//         cancelButtonText: "Continue Shopping",
//       }).then((result) => {
...
//                   <div className="flex justify-between mb-2">
//                     <span>Subtotal ({cardData.length} items):</span>
//                     <span>₹{GET_SUB_TOTAL().toFixed(2)}</span>
//                   </div>
//                   <div className="flex justify-between mb-2">
//                     <span>CGST (4%):</span>
//                     <span>₹{GET_CST_TOTAL("cgst").toFixed(2)}</span>
//                   </div>
//                   <div className="flex justify-between mb-2">
//                     <span>SGST (4%):</span>
//                     <span>₹{GET_CST_TOTAL("sgst").toFixed(2)}</span>
//                   </div>
//                   <div className="flex justify-between font-bold text-lg mt-4 pt-4 border-t border-gray-200">
//                     <span>Order Total:</span>
//                     <span>₹{GET_TOTAL_AMOUNT().toFixed(2)}</span>
//                   </div>
//                 </div>
...
//                         <Radio value="full" className="!flex !items-center !justify-between !py-2 !px-3 !border !rounded-md">
//                           <div>
...
//                         </Radio>
//                         <Radio value="half" className="!flex !items-center !justify-between !py-2 !px-3 !border !rounded-md">
//                           <div>
//                             <span className="font-medium">50% Advance Payment</span>
//                             <div className="text-sm text-gray-600">Pay half now, rest before delivery</div>
//                           </div>
 ...
 const handlePlaceOrder = async (values) => {
  try {
    ...
    const orderData = {
      ...
      remaining_amount: paymentOption === "half" ? (GET_TOTAL_AMOUNT() - values.payment_amount) : 0,
                    ...
                    >
                      <div className="flex flex-col space-y-2">
                        <Radio value="full" className="!flex !items-center !justify-between !py-2 !px-3 !border !rounded-md">
                          ...
                          <span className="font-bold">₹{Number(GET_TOTAL_AMOUNT()).toFixed(2)}</span>
                        </Radio>
                        <Radio value="half" className="!flex !items-center !justify-between !py-2 !px-3 !border !rounded-md">
                          <div>
                            <span className="font-medium">50% Advance Payment</span>
                          ...
                          <span className="font-bold">₹{Number(GET_TOTAL_AMOUNT() * 0.5).toFixed(2)}</span>
                        </Radio>
                      </div>
                    </Radio.Group>
                  </div>
