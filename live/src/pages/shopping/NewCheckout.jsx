...
//                             const cleanValue = value.replace(/\D/g, '');
//                             if (cleanValue.length !== 10) {
//                               return Promise.reject(new Error('Alternate mobile number must be exactly 10 digits'));
//                             }
  ...
  const GET_TAX_TOTAL = () => {
    return _.sum(
      cardData.map((res) => {
        ...
        return Number(res.final_total) * 0.18;
      })
    );
  };

  const GET_TOTAL_AMOUNT = () => {
    return GET_SUB_TOTAL() + GET_TAX_TOTAL();
  };
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="lg:w-1/3 w-full">
              <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200 sticky top-4">
                ...
                <div className="border-t border-gray-200 pt-4">
                  <div className="flex justify-between mb-2">
                    <span>Subtotal<span className="text-sm text-gray-500">({cardData.length} items)</span>:
                    </span>
                    <span>₹{Number(GET_SUB_TOTAL()).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between mb-2">
                    <span>
                      Taxes{" "}
                      <span className="text-sm text-gray-500">(including SGST & CGST 18%)</span>:
                    </span>
