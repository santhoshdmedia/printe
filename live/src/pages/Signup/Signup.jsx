  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      if (response.status === 200) {
        const apiData = response.data; // { data: { user, token }, message }
        const token = _.get(apiData, "data.token", "");
        const user = _.get(apiData, "data.user", {});
        // Save token and user data
        saveTokenToLocalStorage(token);
        localStorage.setItem('userData', JSON.stringify(user));

        // Dispatch with consistent payload structure
        dispatch({
          type: "auth/isSignUpSuccess",
          payload: {
            data: { user, token },
            message: apiData.message || "Google signup successful"
          }
        });
      ...
      const errorMsg = _.get(error, "response.data.message", "Google signup failed. Please try again.");
      dispatch({
        type: "auth/isSignUpFailed",
        payload: errorMsg
      });
    ...
    <div className={`w-full min-h-screen flex !font-primary transition-all duration-500 ${isMounted ? (isExiting ? "exit-animation" : "enter-animation") : "opacity-0"}`}>
      ...
      <div className={`w-full lg:w-1/2 flex items-center justify-center p-3 sm:p-4 md:p-6 lg:p-8 overflow-auto relative transition-all duration-500 ${isMounted ? (isExiting ? "translate-x-full opacity-0" : "translate-x-0 opacity-100") : "translate-x-full opacity-0"}`}>
        ...
        <div className="w-full max-w-md mt-10 sm:mt-12 md:mt-16">
          ...
          <Spin spinning={isSigningUp || isGoogleLoading}>
