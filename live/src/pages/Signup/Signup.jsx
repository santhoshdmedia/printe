...
//code 2
import { Divider, Form, Input, Spin, Tooltip } from "antd";
import React, { useEffect, useState } from "react";
import { FaArrowRight, FaInfoCircle } from "react-icons/fa";
import { FcGoogle } from "react-icons/fc";
import { MdHelpOutline } from "react-icons/md";
...
const Signup = () => {
  ...
  const handleOnChange = (e) => {
    ...
    // For name field, remove extra whitespaces while typing
    if (name === "name") {
  ...
  const nameValidation = (value) => {
    ...
    const pattern = /^[a-zA-Z]+([ ][a-zA-Z]+)*$/;
    const isValid = pattern.test(cleanedName) && cleanedName.length >= 3 && cleanedName.length <= 50;
  ...
  const handleOnSubmit = (e) => {
    ...
    // Clean the name field by removing extra whitespaces
    const cleanedName = form.name.replace(/\s+/g, ' ').trim();
    
    // Update form with cleaned name
    const cleanedForm = {
      ...form,
      name: cleanedName
    };
    ...
    const isConfirmPasswordValid = confirmPasswordValidation(cleanedForm.confirmPassword);
  ...
  return (
    <div className={`w-full min-h-screen flex !font-primary transition-all duration-500 ${isMounted ? (isExiting ? "exit-animation" : "enter-animation") : "opacity-0"}`}>
      {/* Left Section - Signup Form */}
      <div className={`w-full lg:w-1/2 flex items-center justify-center p-4 md:p-8 overflow-auto relative transition-all duration-500 ${isMounted ? (isExiting ? "translate-x-full opacity-0" : "translate-x-0 opacity-100") : "translate-x-full opacity-0"}`}>
        {/* Logo in top left corner with gold background */}
        <div className="absolute top-4 md:top-6 left-4 md:left-6">
          <div className="p-2 md:p-3 bg-yellow-400 flex items-center justify-center rounded-md">
            <img 
              src={logo} 
              alt="Logo" 
          ...
          <Spin spinning={isLogingIn}>
            <form onSubmit={handleOnSubmit} className="bg-white rounded-lg">
              <div className="mb-4 md:mb-6">
                <label className="block text-gray-700 mb-2 font-medium text-sm md:text-base">Full Name</label>
                <Input 
                  value={form.name} 
                  required 
                  name="name" 
              ...
              <div className="mb-4 md:mb-6">
                <label className="block text-gray-700 mb-2 font-medium text-sm md:text-base">Confirm Password</label>
                <Input.Password 
                  value={form.confirmPassword} 
                  required 
                  name="confirmPassword" 
      ...
      <div 
        className={`hidden lg:flex lg:w-1/2 items-center justify-center p-8 fixed right-0 top-0 h-full transition-all duration-500 ${isMounted ? (isExiting ? "-translate-x-full opacity-0" : "translate-x-0 opacity-100") : "-translate-x-full opacity-0"}`}
        style={{
          backgroundImage: `url(${abc})`,
          backgroundSize: 'cover',
          backgroundPosition: 'center'
