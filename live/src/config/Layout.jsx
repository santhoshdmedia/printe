...
const LayoutContent = () => {
  ...
  <div className="sticky top-[40px] z-[999] w-full">
      ...
      {/* ══════════════════════════════════════════════════
          PAGE CONTENT
          ══════════════════════════════════════════════════ */}
      <div className="lg:pt-0 pt-36 overflow-x-hidden max-w-[2000px] mx-auto">
        <Outlet />
      </div>
      ...
      >
        ...
        {/* New Year Image */}
    {/* New Year Image — gift box on mobile, image on desktop */}
<NewYearWidget />
...
const NewYearWidget = () => {
  ...
  return (
    <>
      {/* Fixed widget — bottom left */}
      <div className="fixed bottom-4 left-4 z-[99999]">
        <button
          onClick={() => setClosed(true)}
          className="absolute -top-2 -right-2 w-5 h-5 bg-gray-800 text-white rounded-full text-xs flex items-center justify-center z-10 hover:bg-red-500 transition-all duration-200"
        >✕</button>

        {/* Glow ring */}
        <div className={`absolute inset-0 rounded-2xl transition-all duration-300 ${burst ? 'scale-125 opacity-0' : 'opacity-100'}`}
          style={{ background: 'radial-gradient(circle, rgba(251,191,36,0.3) 0%, transparent 70%)' }} />

        {/* Orbiting stars */}
        <div className="absolute inset-0 pointer-events-none" style={{ animation: 'spin 6s linear infinite' }}>
          {['top-0 left-1/2', 'bottom-0 left-1/2', 'left-0 top-1/2', 'right-0 top-1/2'].map((pos, i) => (
            <span key={i} className={`absolute ${pos} text-yellow-400`} style={{ fontSize: 10 }}>✦</span>
          ))}
        </div>
        ...
        <div
          ...
          style={{ animation: 'float 3s ease-in-out infinite', filter: 'drop-shadow(0 8px 20px rgba(220,38,38,0.4))' }}
        >
          <svg width="110" height="110" viewBox="0 0 180 180" xmlns="http://www.w3.org/2000/svg">
            {/* Sparkles */}
            {[[32,55,'#fbbf24'],[148,60,'#f87171'],[25,135,'#a78bfa'],[155,130,'#34d399']].map(([x,y,c],i)=>(
              <g key={i} style={{ animation: `sparkle-pop 2s ease-in-out ${i*0.4}s infinite` }}>
                <circle cx={x} cy={y} r="3.5" fill={c}/>
                <line x1={x} y1={y-7} x2={x} y2={y+7} stroke={c} strokeWidth="1.5"/>
                <line x1={x-7} y1={y} x2={x+7} y2={y} stroke={c} strokeWidth="1.5"/>
              </g>
            ))}
            {/* Lid */}
            <g style={{ animation: 'lid-peek 4s ease-in-out 1s infinite', transformOrigin: 'center bottom' }}>
              <rect x="44" y="66" width="92" height="26" rx="5" fill="#dc2626"/>
              <rect x="82" y="66" width="16" height="26" fill="#ef4444" opacity="0.8"/>
              {/* Bows */}
              <ellipse cx="72" cy="68" rx="18" ry="10" fill="#fbbf24" transform="rotate(-20,72,68)" style={{ animation: 'wiggle 2s ease-in-out infinite', transformOrigin: 'right center' }}/>
              <ellipse cx="108" cy="68" rx="18" ry="10" fill="#fbbf24" transform="rotate(20,108,68)" style={{ animation: 'wiggle 2s ease-in-out infinite reverse', transformOrigin: 'left center' }}/>
              <ellipse cx="90" cy="68" rx="9" ry="9" fill="#facc15"/>
              <circle cx="90" cy="68" r="3" fill="#fff" opacity="0.6"/>
            </g>
            {/* Box */}
            <rect x="44" y="90" width="92" height="64" rx="5" fill="#ef4444"/>
            <rect x="82" y="90" width="16" height="64" fill="#fbbf24" opacity="0.9" style={{ animation: 'ribbon-pulse 2s ease-in-out infinite' }}/>
            <rect x="44" y="148" width="92" height="8" rx="3" fill="#b91c1c" opacity="0.6"/>
          </svg>
          <p className="text-center text-xs text-yellow-500 font-medium mt-1 tracking-wide"
            style={{ animation: 'sparkle-pop 1.5s ease-in-out 2s infinite' }}>
            tap to reveal
          </p>
        </div>
      </div>
      ...
      {fullscreen && (
        ...
        >
          <button
            onClick={() => setFullscreen(false)}
            className="absolute top-5 right-5 w-11 h-11 rounded-full bg-white/20 hover:bg-white/40 text-white text-xl flex items-center justify-center transition-all duration-200 hover:rotate-90"
          >✕</button>
          <div style={{ animation: 'bounce-in 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards' }}
            ...
            <img
              ...
              style={{ boxShadow: '0 0 60px rgba(251,191,36,0.3), 0 30px 80px rgba(0,0,0,0.5)' }}
            />
            <p className="text-center text-yellow-400 mt-4 text-sm tracking-widest font-medium"
              style={{ animation: 'sparkle-pop 1.5s ease-in-out infinite' }}>
              ✦ Happy New Year ✦
            </p>
          </div>
        </div>
      ...
      <style>{`
        ...
        @keyframes sparkle-pop { 0%,100%{transform:scale(0);opacity:0} 50%{transform:scale(1);opacity:1} }
        @keyframes ribbon-pulse { 0%,100%{transform:scaleX(1)} 50%{transform:scaleX(1.08)} }
        @keyframes lid-peek { 0%,70%,100%{transform:translateY(0) rotate(-2deg)} 35%{transform:translateY(-20px) rotate(-6deg)} }
        @keyframes bounce-in { 0%{transform:scale(0.3);opacity:0} 60%{transform:scale(1.12)} 80%{transform:scale(0.96)} 100%{transform:scale(1);opacity:1} }
