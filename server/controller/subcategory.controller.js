...
const addsub = async (req, res) => {
  try {
    // The slug will be automatically generated by the pre-save middleware
    const result = await SubcategorySchema.create(req.body);
    
    // Return response with generated slug
    successResponse(res, "Subcategory added successfully", {
      id: result._id,
      sub_category_name: result.sub_category_name,
      slug: result.slug, // This will contain the auto-generated slug
      select_main_category: result.select_main_category,
    ...
    if (err.code === 11000 && err.keyPattern && err.keyPattern.slug) {
      return errorResponse(res, "Slug already exists. Please use a different subcategory name.");
    }
    
    // Handle validation errors
    if (err.name === 'ValidationError') {
      const errors = Object.values(err.errors).map(error => error.message);
      return errorResponse(res, `Validation error: ${errors.join(', ')}`);
    }
    
    return errorResponse(res, "Failed to add subcategory");
  }
};

// Optional: Helper function if you need to manually generate slugs elsewhere
const generateSlug = async (subCategoryName, excludeId = null) => {
  ...
  while (!isUnique) {
    ...
    const existing = await SubcategorySchema.findOne(query);
