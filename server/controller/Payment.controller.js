...
const validateCCAvenueCredentials = () => {
  ...
  if (!CCA_MERCHANT_ID || !CCA_WORKING_KEY || !CCA_ACCESS_CODE) {
    return { valid: false, error: 'Payment gateway not configured', credentials: null };
  }
  return {
    valid: true,
    error: null,
...
const adminCreateOrder = async (req, res) => {
  try {
    ...
    const ccavenueParams = {
      ...
      billing_address:  (processedAddress.street || '').substring(0, 100),
      billing_city:     processedAddress.city    || '',
      billing_state:    processedAddress.state   || '',
      billing_zip:      processedAddress.pincode || '',
      billing_country:  processedAddress.country || 'India',
      delivery_name:    customer_name.substring(0, 50),
      delivery_address: (processedAddress.street || '').substring(0, 100),
      delivery_city:    processedAddress.city    || '',
      delivery_state:   processedAddress.state   || '',
      delivery_zip:     processedAddress.pincode || '',
...
const qrRedirectToGateway = async (req, res) => {
  try {
    ...
    if (!encryptedData || !storedAccessCode) {
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .container { background: white; padding: 50px 40px; border-radius: 24px; box-shadow: 0 25px 70px rgba(0,0,0,0.4); max-width: 500px; width: 100%; text-align: center; animation: slideIn 0.6s ease-out; }
    @keyframes slideIn { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
    .spinner { border: 5px solid #f3f3f3; border-top: 5px solid #667eea; border-radius: 50%; width: 70px; height: 70px; animation: spin 1s linear infinite; margin: 0 auto 30px; }
    @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
    h2 { color: #2d3748; margin: 0 0 15px; font-size: 28px; font-weight: 800; }
    .subtitle { color: #718096; margin: 0 0 35px; font-size: 16px; }
    .order-info { background: linear-gradient(135deg, #f7fafc 0%, #edf2f7 100%); padding: 25px; border-radius: 16px; margin-top: 30px; border: 2px solid #e2e8f0; }
    .order-row { display: flex; justify-content: space-between; align-items: center; margin: 12px 0; padding: 10px 0; }
    .order-row:not(:last-child) { border-bottom: 1px solid #e2e8f0; }
    .label { color: #4a5568; font-weight: 600; font-size: 15px; }
    .value { color: #2d3748; font-weight: 700; font-size: 15px; }
    .amount { color: #667eea; font-size: 32px; font-weight: 900; }
    .security { display: flex; align-items: center; justify-content: center; margin-top: 30px; padding: 15px; background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); border-radius: 12px; border: 2px solid #86efac; }
    .security-text { color: #166534; font-size: 14px; font-weight: 700; margin-left: 10px; }
    .progress { width: 100%; height: 5px; background: #e2e8f0; border-radius: 3px; overflow: hidden; margin-top: 25px; }
    .progress-fill { height: 100%; background: linear-gradient(90deg, #667eea, #764ba2); animation: progress 1.5s ease-in-out; }
    @keyframes progress { from { width: 0%; } to { width: 100%; } }
    .gateway { margin-top: 20px; padding-top: 20px; border-top: 2px solid #e2e8f0; }
    .gateway p { color: #718096; font-size: 13px; margin-bottom: 8px; }
    .badge { display: inline-block; background: #003366; color: white; padding: 8px 20px; border-radius: 8px; font-weight: 700; font-size: 14px; }
  </style>
</head>
<body>
  <div class="container">
  ...
  <script>
    ...
    setTimeout(() => {
      ...
      btn.onclick = () => document.getElementById('paymentForm').submit();
...
const getPaymentStatus = async (req, res) => {
  try {
    ...
    let order = await OrderDetailsSchema.findOne({ invoice_no: order_id });
    if (!order) order = await OrderDetailsSchema.findById(order_id).catch(() => null);
...
const getAllOrders = async (req, res) => {
  try {
    ...
    const orders = await OrderDetailsSchema
      .find(query)
...
const regenerateQRCode = async (req, res) => {
  try {
    ...
    const encryptedData = new CCAvenue(workingKey).encryptData(ccavenueParams);
