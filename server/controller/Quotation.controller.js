...
const adminCreateQuotation = async (req, res) => {
  try {
    ...
    return successResponse(res, "Quotation created", {
      ...
      pdf_link: `${BACKEND_BASE_URL}/api/quotation/pdf/${savedQuotation.quotation_no}`,
      qr_code: savedQuotation.qr_code || null,
      qr_url: savedQuotation.qr_url || null,
      user_id: userId,
    });
  } catch (err) {
    console.error("✗ adminCreateQuotation error:", err);
...
const getQuotationByNumber = async (req, res) => {
  try {
    ...
    return successResponse(res, "Quotation retrieved", {
      ...
      qr_code: quotation.qr_code || null,
      qr_url: quotation.qr_url || null,
      createdAt: quotation.createdAt,
    });
  } catch (err) {
    console.error("✗ getQuotationByNumber error:", err);
...
const getAllQuotations = async (req, res) => {
  try {
    ...
    const [quotations, total] = await Promise.all([
      ...
      QuotationSchema.countDocuments(query),
    ]);
...
const getQuotationStatus = async (req, res) => {
  try {
    ...
    if (!quotation) {
      // Fallback: maybe an _id was passed instead of quotation_no
      quotation = await QuotationSchema.findById(quotation_no).catch(() => null);
    }
    if (!quotation) return errorResponse(res, "Quotation not found");
    ...
    console.error("✗ getQuotationStatus error:", err);
...
const regenerateQuotationQR = async (req, res) => {
  try {
    ...
    console.error("✗ regenerateQuotationQR error:", err);
